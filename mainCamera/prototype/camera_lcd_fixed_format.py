...
def main():
    try:
        ...
        # Show startup screen
        startup_image = Image.new("RGB", (240, 320), "BLACK")
        draw = ImageDraw.Draw(startup_image)
        draw.text((50, 140), "Camera Starting...", fill="WHITE")
        draw.text((60, 160), "IMX708 Ready", fill="GREEN")
        ...
        # Configure camera for LCD display (240x320) with correct format
        config = camera.create_preview_configuration(
            main={"size": (240, 320), "format": "RGB888"},
            buffer_count=2
        )
        ...
        logging.info("Starting camera preview on LCD...")
        ...
        while True:
            try:
                ...
                draw.text((5, 45), "Format Fixed", fill="CYAN")
                draw.text((5, 295), f"Frame: {frame_count}", fill="GREEN")
        ...
        try:
            # Show exit screen
            exit_image = Image.new("RGB", (240, 320), "BLACK")
            draw = ImageDraw.Draw(exit_image)
            draw.text((70, 140), "Camera", fill="WHITE")
            draw.text((85, 160), "Test", fill="GREEN")
            draw.text((70, 180), "Complete!", fill="YELLOW")
            disp.ShowImage(exit_image)
            time.sleep(3)
            disp.clear()

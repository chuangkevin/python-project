...
- [x] **階段0.5**: 基礎模組完善 - ✅ **已完成**
  ...
  - ✅ 硬體模擬器整合
- [x] **階段1**: UI 設計實現 - ✅ **已完成**
  - ✅ 基於 analogGauge 風格設計相機控制界面
  - ✅ 創建雙螢幕顯示系統（主矩形 + 圓形副螢幕）
  - ✅ 實現狀態機驅動的動態圓形錶盤切換
  - ✅ **完整雙螢幕UI系統**：`systemControl/ui/fixed_dual_screen_system.py`
    - 主螢幕：Live View + 圓形導航輪盤（旋轉時顯示，3秒後自動隱藏）
    - 圓形螢幕：動態切換 RD-1 整合錶盤 ↔ 專用模式錶盤
    - 左編碼器：狀態切換（旋轉預覽 + 按下確認）
    - 右編碼器：參數調整
    - 雙重回調機制：即時預覽 + 最終選擇
- [ ] **階段1.5**: UI 美化優化 - 規劃中
  - [ ] 完成各狀態專用錶盤設計 (軟片模擬、白平衡、ISO、對焦等)
  - [ ] 美化雙螢幕UI界面 (色彩、字體、排版)
  - [ ] 優化圓形錶盤視覺效果 (指針、刻度、動畫)
  - [ ] 改善主螢幕Live View顯示 (更真實的預覽效果)
  - [ ] 完善圓形導航輪盤美感 (圖示、高光、漸層)
- [ ] **階段2**: 相機模組整合 - 待開始
- [ ] **階段3**: 硬體 GPIO 整合 - 待開始

### 📋 當前開發狀況 (2025-09-08)
- ✅ **Python 單體應用架構**: 完成重構，基於 analogGauge 風格的專業相機控制界面
- ✅ **硬體模擬器**: 整合完成，支援開發模式測試
- ✅ **設定系統**: 統一管理相機、顯示、轉盤、軟片等設定
- ✅ **完整雙螢幕系統**: 主矩形螢幕 Live View + 圓形副螢幕動態錶盤
- ✅ **狀態機 UI**: 基於左編碼器切換的動態圓形錶盤系統
...
│   ├── ui/              # 設定界面 ✅
│   │   ├── fixed_dual_screen_system.py  # 完整雙螢幕UI系統 ✅
...
### 雙螢幕系統
- **主矩形螢幕**：2.4" ILI9341 TFT LCD (SPI, 240×320)  
  - 主要功能：Live View 即時預覽
  - 狀態切換時：浮現圓形導航界面 (2秒後消失)
- **圓形副螢幕**：0.71" GC9D01 圓形 LCD (SPI, 240×240)  
  - **Default 狀態**：四錶盤整合顯示 (`draw_integrated_rd1_display()`)
  - **特定狀態**：對應專用圓形錶盤（軟片模擬/畫質/白平衡/ISO/對焦等）  

### 控制元件
- **五向搖桿**：導航選單 (含2個擴展鍵)
- **兩段式快門**：半按 AF，全按拍照 (可自訂功能，如白卡測光)
- **左編碼器 (狀態控制)**：
  - **轉動**：切換主要拍攝狀態 (軟片模擬→畫質→白平衡→比例→ISO→對焦...)
  - **按下**：回到 Default 狀態 (顯示四錶盤整合)
  - **狀態切換時**：主螢幕浮現導航圓盤 (如圖片所示，2秒後消失)
- **右編碼器 (參數調整)**：
  - **轉動**：調整當前狀態的數值 (如 ISO 100→200→400...)
  - **按下**：確認/進入子選單
...
```bash
cd systemControl/ui
python fixed_dual_screen_system.py
```
**功能說明**：
- **三個獨立窗口**：模擬雙實體螢幕 + 控制面板
- **左編碼器**：旋轉切換狀態（即時預覽圓形螢幕內容），按下確認選擇
- **右編碼器**：調整當前狀態參數
- **主螢幕導航**：狀態切換時浮現圓形輪盤，3秒後自動隱藏
- **圓形螢幕**：動態切換 RD-1 整合錶盤 ↔ 專用錶盤
...
#### **狀態切換機制**
1. **Default 狀態** (預設)
   - **圓形螢幕**：顯示四錶盤整合 (`draw_integrated_rd1_display()`)
     - 剩餘拍攝數、白平衡、電池電量、影像品質
   - **主螢幕**：Live View 即時預覽

2. **狀態切換操作**
   - **左編碼器轉動** → 切換拍攝狀態
   - **主螢幕浮現導航圓盤** (如上圖所示，持續 2 秒後消失)
   - **圓形螢幕切換** → 顯示對應狀態的專用錶盤

3. **專用狀態模式**
   - **軟片模擬**：圓形錶盤顯示 Fujifilm 軟片效果選項
   - **相片畫質**：圓形錶盤顯示解析度/壓縮設定
   - **白平衡**：圓形錶盤顯示色溫/場景模式
   - **照片比例**：圓形錶盤顯示 4:3、16:9、1:1 等比例
   - **ISO**：圓形錶盤顯示感光度數值 (100-6400)
   - **對焦模式**：圓形錶盤顯示單點、連續、手動等模式
   - **系統設定**：特殊狀態，主螢幕不顯示 Live View
